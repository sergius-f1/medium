...
import { CONTENT_TYPE_JSON, HEADER_AUTHORIZATION, HEADER_CONTENT_TYPE, HTTP_STATUS, HttpMethod } from "./constants";

export class ApiError extends Error {
  constructor(public readonly status: number, public readonly errors: Record<string, string[]>) {
 ...
 * @param path - endpoint path
 */
const deleteMethod = <T>(path: string): Promise<T> => request<T>("DELETE", path);
